...
export default function ApprenticeshipsPage() {
  const apprenticeships = MOCK_OPPORTUNITIES.filter(o => o.type === 'Apprenticeship');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          <div className="space-y-3">
            <span className="text-xs font-bold uppercase tracking-wider text-amber-800 bg-amber-50 px-3 py-1 rounded-full border border-amber-200">
              Structured Industry Mentorship
            </span>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-[#0A1428] tracking-tight">
              Learn While Earning Apprenticeships
            </h1>
            <p className="text-slate-600 text-sm max-w-2xl">
              Combine structured daily industry mentorship with stipend-backed practical training programs.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {apprenticeships.map((opp) => (
